...
def create_emp():
    try:
        ...
        if _name and _email and _phone and _address and request.method == 'POST':
            ...
            binData = (_name, _email, _phone, _address)
            conn = mysql.connect()
...
def update_emp():
    try:
        ...
        if _name and _email and _phone and _address and _id and request.method == 'PUT':
            ...
            binData = (_name, _email, _phone, _address, _id)
            conn = mysql.connect()
            cursor = conn.cursor()
            cursor.execute(sqlQuery, binData)
            conn.commit()
            respone = jsonify("Employee Update Successfully")
            respone.status_code = 200
            return respone
        else:
            return showMessage()
...
def delete_emp(id):
    try:
        ...
        conn.commit()
        respone = jsonify("Employee Delete Successfully")
        respone.status_code = 200
        return respone
    except Exception as e:
        print(e)
